...
async function init() {
    ...
    let root = document.querySelector('.choices');
...
function buildList(jobs) {
    ...
    for (let job of jobs) {
        ...
        html += `<button id="${job.occupation}" data-salary="${job.salary}">${job.occupation}</button>`;
        //create a div element for the salary and format it as currency
        html += `<p>$${job.salary.toLocaleString('en-US')}</p>`;
        //close the section
        html += '</section>';
    }
